...
export default function CategoryPage() {
  ...
  const categoryPhotos = photos.filter(p => p.category === category || p.folder === category);
  
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const [lightboxIndex, setLightboxIndex] = useState(0);
  const gallery = categoryPhotos.map(p => ({ public_url: p.image_url, file_name: p.title || 'Photo' }));
...
with open('src/app/photos/[category]/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print('Category page written successfully')
